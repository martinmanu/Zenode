import { ShapeRenderer } from "../../types/index.js";
import { trianglePath } from "../geometry/trianglePath.js";

/**
 * Triangle shape renderer using SVG <path>.
 */
export const TriangleRenderer: ShapeRenderer = {
  draw(group, config) {
    const { color, stroke, transparency } = config;
    group
      .append("path")
      .attr("d", trianglePath(config))
      .attr("fill", color)
      .attr("fill-opacity", transparency ?? 1)
      .attr("stroke", stroke.color)
      .attr("stroke-width", stroke.width)
      .attr(
        "stroke-dasharray",
        (stroke as any).strokeDasharray?.length
          ? (stroke as any).strokeDasharray.join(" ")
          : null
      );
  },

  getPath(config) {
    return trianglePath(config);
  },

  getBounds(config) {
    const { x, y, width, height } = config;
    return { x, y, width, height };
  },

  getPorts(config) {
    const { x, y, width, height } = config;
    return {
      peak: { x: x + width / 2, y: y },
      bottom_left: { x: x, y: y + height },
      bottom_right: { x: x + width, y: y + height },
      center: { x: x + width / 2, y: y + height / 2.5 }, // Adjust for visual center
    };
  },
};
