...
export function getCurvedPath(params: PathParams): string {
  ...
  const getHandleOffset = (portId?: string, isSource: boolean = true) => {
    // If no port specified, we try to guess based on relative position
    if (!portId || portId === 'center') {
        const factor = isSource ? 1 : -1;
        if (dx > dy) return { x: (target.x > source.x ? handleLength : -handleLength) * factor, y: 0 };
