...
import { D3Selection, VisualState } from "@zenode/core";
...
function ensureEffectsStyle(): void {
...
function ensureDefs(svg: d3.Selection<SVGSVGElement, unknown, null, undefined>) {
  let defs = svg.select<SVGDefsElement>("defs.zenode-effects-defs");
  if (defs.empty()) {
...
function ensureGlowFilter(
  ...
  if (!defs.select(`#${filterId}`).empty()) return filterId;
...
export function applyEffects(
  ...
  const effects = visualState?.effects;
  if (!effects) return;
  ...
  if (effects.glow) {
    const intensity = Math.max(0.1, effects.glow.intensity ?? 1);
    const color = effects.glow.color ?? "var(--zenode-status-running, #5DADE2)";
  ...
  if (effects.strokeAnimation?.type === "flow") {
    const speed = Math.max(0.2, effects.strokeAnimation.speed ?? 1);
  ...
  // --- NODE STATUS EFFECTS ---
  if (visualState?.status && visualState.status !== "idle") {
